...
-- Season cleanup: permanently deletes a season and everything under it
-- (teams/rosters, coach assignments, games, pitching/catching logs,
-- attendance/absences). See database/migrations/add_delete_season_cascade_function.sql
-- for the full rationale -- teams.season_id, games.season_id, and
-- players.team_id are ON DELETE RESTRICT on purpose, so this function
-- deletes in dependency order (games, then players, then teams, then the
-- season) inside one transaction instead of relying on cascade.
...
-- =====================================================
-- Applied via: database/migrations/enable_query_monitoring.sql
-- Reference:   database/PERFORMANCE_DECISIONS.md
--
-- NOTE: The former `public.rls_query_performance` view (a dev-only monitor over
-- pg_stat_statements) was removed to resolve the Supabase Security Advisor
-- `security_definer_view` finding. See fix_security_advisor_warnings.sql. To
-- monitor RLS-heavy queries, query pg_stat_statements directly. The
-- pg_stat_statements extension and reset_query_stats() function are retained.
--
-- The orphaned `constraint_name_var` table was likewise dropped by that same
-- migration (Advisor `rls_disabled_in_public` finding).
-- =====================================================

-- Enable pg_stat_statements extension for query profiling
CREATE EXTENSION IF NOT EXISTS pg_stat_statements;

-- Function to reset pg_stat_statements (super_admin only)
...
-- instead of (select public.is_super_admin()). To apply the subquery
-- wrapper for performance consistency, run:
--
--   DROP POLICY "Only super_admins can update app config" ON public.app_config;
--   CREATE POLICY "Only super_admins can update app config"
